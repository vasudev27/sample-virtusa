...
@SpringBootTest
public class NumToWordConvApplicationTests {
	...
	public void rejectNumbersOver999999999() {
		WordConverter.convertNumToWord(1000000000);
	}
	
	@Test(expected = IllegalArgumentException.class)
	public void rejectNegitiveNumbers() {
		WordConverter.convertNumToWord(-5);
	}

	private void expectConversion(final int number, final String expectedWords) {
		assertEquals(expectedWords, WordConverter.convertNumToWord(number));
	}
